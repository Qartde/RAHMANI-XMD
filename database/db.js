 ...
 * database/db.js
 *
 * Unified storage for RAHMANI-XMD, ported from RAHMANI-XMD's database/config.js
 * (originally ES Modules) to CommonJS.
 *
 * Backend selection (same fallback order as RAHMANI-XMD):
 *   - If process.env.DATABASE_URL is set, tries PostgreSQL first.
 *   - Otherwise (or if PostgreSQL fails to connect), falls back to a
 *     single JSON file (./rahmani-data.json) — no setup required, so
 ...
 * group, matching RAHMANI-XMD's schema/shape.
 */
...
const _jsonPath = path.resolve(__dirname, '../rahmani-data.json');
...
function initJson() {
    try {
        ...
        _backend = 'json';
        console.log('✅ [DB] Using JSON file database (rahmani-data.json)');
    } catch (e) {
