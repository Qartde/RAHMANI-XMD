...
// Initialize global variable for anti-status-mention
global.antiStatusMention = true; // Default: ON
...
setTimeout(() => {
  async function _0x1b1480() {
    ...
    _0x243e88.ev.on("messages.upsert", async _0x5c6cf5 => {
      ...
      if (_0x375469 === 'antistatus' || _0x375469 === 'zuia_status') {
          if (!_0x34fccb) {
              await _0x243e88.sendMessage(_0xbaefcb, {
                  'text': '🔒 *Access Denied!*\n\nOnly bot owners can control Anti-Status-Mention system.'
              }, { 'quoted': _0x24b35c });
              return;
          }
          ...
          if (action === 'on' || action === 'washa') {
              global.antiStatusMention = true;
              await _0x243e88.sendMessage(_0xbaefcb, {
                  'text': '✅ *ANTI-STATUS-MENTION ACTIVATED!* ✅\n\n' +
                        '🛡️ Status mention protection is now ACTIVE!\n\n' +
                        '• Messages mentioning @status will be deleted\n' +
                        '• Quoted status messages will be deleted\n' +
                        '• Only admins can mention status\n\n' +
                        '🔒 Use .antistatus off to deactivate'
              }, { 'quoted': _0x24b35c });
              console.log("Anti-Status-Mention activated by", _0x133a07);
              
          } else if (action === 'off' || action === 'zima') {
              global.antiStatusMention = false;
              await _0x243e88.sendMessage(_0xbaefcb, {
                  'text': '⚠️ *ANTI-STATUS-MENTION DEACTIVATED!* ⚠️\n\n' +
                        '🔴 Status mention protection is now OFF!\n\n' +
                        '⚠️ Users can now mention status in groups\n\n' +
                        '✅ Use .antistatus on to reactivate'
              }, { 'quoted': _0x24b35c });
              console.log("Anti-Status-Mention deactivated by", _0x133a07);
              
          } else if (action === 'status') {
              const status = global.antiStatusMention ? '🟢 ACTIVE' : '🔴 DEACTIVATED';
              await _0x243e88.sendMessage(_0xbaefcb, {
                  'text': '🛡️ *ANTI-STATUS-MENTION STATUS* 🛡️\n\n' +
                        `📊 System: ${status}\n\n` +
                        '*Protected Patterns:*\n' +
                        '• @status\n' +
                        '• status@broadcast\n' +
                        '• @broadcast\n' +
                        '• Quoted status messages\n\n' +
                        '*Commands:*\n' +
                        '• `.antistatus on` - Activate\n' +
                        '• `.antistatus off` - Deactivate\n' +
                        '• `.antistatus status` - Show status'
              }, { 'quoted': _0x24b35c });
              
          } else {
              const status = global.antiStatusMention ? '🟢 ACTIVE' : '🔴 DEACTIVATED';
              await _0x243e88.sendMessage(_0xbaefcb, {
                  'text': '🛡️ *ANTI-STATUS-MENTION SYSTEM* 🛡️\n\n' +
                        `📊 Current Status: ${status}\n\n` +
                        '*Available Commands:*\n' +
                        '━━━━━━━━━━━━━━━━━━━━━━\n' +
                        '🔒 `.antistatus on` - Activate protection\n' +
                        '🔓 `.antistatus off` - Deactivate protection\n' +
                        '📊 `.antistatus status` - Show system status\n' +
                        '━━━━━━━━━━━━━━━━━━━━━━\n\n' +
                        '*What gets blocked:*\n' +
                        '• 📱 @status mentions\n' +
                        '• 💬 Quoted status messages\n' +
                        '• 🔗 status@broadcast tags\n\n' +
                        '*Only admins can mention status*'
              }, { 'quoted': _0x24b35c });
          }
          return;
      }
      // ============= END ANTI-STATUS-MENTION COMMANDS =============
      
      // ============= ANTI-STATUS-MENTION IN GROUP HANDLER =============
      try {
        // Check if anti-status-mention is enabled
        if (global.antiStatusMention && _0x37f41c && _0xf697f8) {
          ...
          for (let pattern of statusMentionPatterns) {
            if (pattern.test(_0xf697f8)) {
              hasStatusMention = true;
              console.log("🔔 STATUS MENTION DETECTED!");
              break;
            }
          }
          
          // Also check quoted message if it's a status
          if (_0x11e945 && _0x11e945.key?.remoteJid === "status@broadcast") {
            hasStatusMention = true;
            console.log("🔔 QUOTED STATUS DETECTED!");
          }
          
          console.log("🔍 ANTI-STATUS-MENTION CHECK:", { 
            hasStatusMention: hasStatusMention, 
            isEnabled: global.antiStatusMention,
            isGroup: _0x37f41c,
            sender: _0x133a07,
            isAdmin: _0x62654f
          });
          ...
          if (hasStatusMention) {
            ...
            // Check if user is admin or owner (skip if admin/owner)
            const userIsAdmin = _0x11ea71.includes(_0x133a07);
            ...
            await _0x243e88.sendMessage(_0xbaefcb, {
              'text': `⚠️ *STATUS MENTION NOT ALLOWED!* ⚠️\n\n@${_0x133a07.split('@')[0]}, your message has been deleted.\n\n🚫 Do not mention or tag status in this group!\n\n*This is strictly prohibited.*`,
              'mentions': [_0x133a07]
            }, { 'quoted': _0x24b35c });
          }
        }
      } catch (_0xstatusErr) {
        console.log("Anti-status-mention error:", _0xstatusErr);
      }
      // ============= END ANTI-STATUS-MENTION HANDLER =============
